...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			// if (empty($product_info['canonical'])) {
			//	$this->document->setCanons('caaaanonicaaaalufffrl');
			// }
			
			if ($product_info['noindex'] <= 0 && $this->config->get('config_noindex_status')) {
				$this->document->setRobots('noindex,follow');
			} elseif ($product_info['noindex'] = 3 && $this->config->get('config_noindex_status')) {
			...
			if (!is_null($product_info['special']) && (float)$product_info['special'] >= 0) {
				...
				$data['specialnull'] = $product_info['special'];
			...
			foreach ($results as $result) {
			    ...
			    if ($result['pmtemplate'] == 'productpol') {
    				if ($result['image']) {
    					$image = $this->model_tool_imagequadr->resize($result['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_related_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_related_height'));
    				} else {
