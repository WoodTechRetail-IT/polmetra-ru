	    ...
	    if (!$this->webp) {
			if (isset($this->jetcache_settings['image_status']) && $this->jetcache_settings['image_status'] && isset($this->jetcache_settings['image_webp_status']) && $this->jetcache_settings['image_webp_status']) {
				if (((isset($this->request->server['HTTP_ACCEPT']) && strpos($this->request->server['HTTP_ACCEPT'], 'image/webp') !== false)
					...
					|| (isset($this->request->server['HTTP_USER_AGENT']) && strpos(strtolower($this->request->server['HTTP_USER_AGENT']), 'lighthouse') !== false)
			...
			if (isset($this->jetcache_settings['minify_css_status']) && $this->jetcache_settings['minify_css_status'] ||
				...
				if (!$this->jc_ajax && isset($this->jetcache_settings['jetcache_widget_status']) && $this->jetcache_settings['jetcache_widget_status']) {
					...
					if (stripos($this->outputHTML, '</body>') === false) {
						$this->log->write('Jet Cache: where </body> ?' . PHP_EOL . $this->request_uri_trim . PHP_EOL . $this->route . PHP_EOL . $ajax_log . PHP_EOL . $this->request->server['HTTP_USER_AGENT'] . PHP_EOL);
						return $this->outputHTML;
					}
					if (stripos($this->outputHTML, '</head>') === false) {
						$this->log->write('Jet Cache: where </head> ?' . PHP_EOL . $this->request_uri_trim . PHP_EOL . $this->route . PHP_EOL . $ajax_log . PHP_EOL . $this->request->server['HTTP_USER_AGENT'] . PHP_EOL);
						return $this->outputHTML;
					}
...
$js_js = $js_js . $minify_js_preload_desktop;
	...
	public function jetcache_minify_js_preload_gps($output) {
		if (isset($this->request->server['HTTP_USER_AGENT']) && (strpos(strtolower($this->request->server['HTTP_USER_AGENT']), 'lighthouse') !== false || strpos(strtolower($this->request->server['HTTP_USER_AGENT']), 'google page speed') !== false)) {
	...
	private function set_cache_name($settings) {
        ...
        if ($settings_type != 'query') {
            ...
            /* Debug
            if (!isset($this->registry->get('request')->get['route']) || $this->registry->get('request')->get['route'] != 'jetcache/tool/get_buildcache_array') {
            	print_my($data_cache);
            	print_my($hash);
            }
            */
