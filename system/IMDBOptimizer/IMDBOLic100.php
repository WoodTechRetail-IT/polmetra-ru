...
class IMDBOLic100 
{
	...
	function __construct(&$controller, $module_name, $publickey, $enc_message)
	{
...
-----END PUBLIC KEY-----';
		...
		$this->dec_message = base64_decode(base64_decode(base64_decode(base64_decode(base64_decode(base64_decode($this->enc_message))))));
		...
		if (
			...
			$currset = $this->controller->model_setting_setting->getSetting($this->module_name . 'DataDemo');
			
			if (!isset($currset[$this->module_name . 'DataDemo_date'])) {
				$currset = array();
				$date_demo_now = new DateTime("now");
				$date_demo_now->add(new DateInterval('P5D'));
				$date_demo_now_str = $date_demo_now->format('Y-m-d');
				$this->rsa->loadKey($this->demo_privatekey);
				$date_demo_now_str = base64_encode($date_demo_now_str . ':::' . base64_encode($date_demo_now_str));
				$currset[$this->module_name . 'DataDemo_date'] = base64_encode($this->rsa->encrypt($date_demo_now_str));
				if (method_exists($this->controller->model_setting_setting, 'editSetting')
					|| method_exists('ModelSettingSetting', 'editSetting')) {
					$this->controller->model_setting_setting->editSetting($this->module_name . 'DataDemo', $currset);
				} else {
					error_reporting($currLevelReport);
					return;
				}
