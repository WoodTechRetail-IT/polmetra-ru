...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			return array(
				...
				'description'      => $query->row['description'],
				'pmtemplate'      => $query->row['pmtemplate'],
	...
	public function getProducts($data = array()) {
        ...
        if (is_array($data['filter_category_id'])) {
            foreach ($data['filter_category_id'] as $filter_category_id) {
                $filter_categories[] = (int)$filter_category_id;
            }
        } else {
            $filter_categories[] = (int)$data['filter_category_id'];
        }
      
				$sql .= " AND cp.path_id  IN (" . implode(',', $filter_categories) . ")";
			} else {
        ...
        if (is_array($data['filter_category_id'])) {
            ...
            $filter_categories[] = (int)$data['filter_category_id'];
				...
				$sql .= " AND p2c.category_id  IN (" . implode(',', $filter_categories) . ")";
			}
    ...
    // OCFilter end
        ...
        if (!empty($data['attributes'])) {
            foreach ($data['attributes'] as $attribute_id => $attributes) {
                $atr_implode = array();
                foreach ($attributes as $attribute) {                    
                    $atr_implode[] = "pafp.text LIKE '" . $this->db->escape($attribute) . "'";
                }
                if ($atr_implode) {
                    $sql .= " AND (SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "product_attribute` pafp WHERE pafp.product_id = p.product_id AND pafp.attribute_id = " . (int)$attribute_id . " AND (" . implode(' OR ', $atr_implode) . ")) > 0";
                }
            }
        }
        if (!empty($data['filter_product_id'])) {
            $filter_product_id = array();
            foreach ($data['filter_product_id'] as $fpid) {
                $filter_product_id[] = (int)$fpid;
            }
            $sql .= " AND p.product_id IN (" . implode(',', $filter_product_id) . ")";
        }
        ...
        if (is_array($data['filter_manufacturer_id'])) {
            foreach ($data['filter_manufacturer_id'] as $filter_manufacturer_id) {
                $filter_manufacturers[] = (int)$filter_manufacturer_id;
            }
        } else {
            $filter_manufacturers[] = (int)$data['filter_manufacturer_id'];
        }
      
			$sql .= " AND p.manufacturer_id  IN (" . implode(',', $filter_manufacturers) . ")";
		}
