...
class ControllerExtensionModuleSeoTagsGenerator extends Controller {
	...
	public function getCategoryTags($category_info) {
		...
		// For OCFilter . Begin
		if ( (version_compare(VERSION, '3') >= 0 ) && ( (isset($category_info['stg_flag']) && 'oc_filter' == $category_info['stg_flag']) && (is_object($this->ocfilter) && !$this->ocfilter->getParams() || is_object($this->ocfilter) && $this->ocfilter->getPageInfo()) ) ) {
			$a_specific_formula = array(
				'title'				 => $category_info['meta_title'],
				'description'	 => $category_info['meta_description'],
				'keyword'			 => $category_info['meta_keyword'],
			...
			// A! h1 is KEY in $ocfilter_page, it is not corelated with db field for category
			if (isset($category_info['h1'])) {
				$a_specific_formula['h1'] = $category_info['h1'];
			}
		}
		// For OCFilter . End
		...
		if ($this->isFollowedVar('city', $formulas_array)) {
			$config_store_city	= $this->config->get('config_store_city');
			$config_store_city	= $config_store_city[$lang_id];
		...
		// For OCFilter . Begin
		if ( (version_compare(VERSION, '3') >= 0 ) && ( is_object($this->ocfilter) && !$this->ocfilter->getParams() ) ) {
			$category_info['meta_title'] = str_replace('{filter}', '', $category_info['meta_title']);
			$category_info['meta_description'] = str_replace('{filter}', '', $category_info['meta_description']);
			$category_info['meta_keyword'] = str_replace('{filter}', '', $category_info['meta_keyword']);
			
			if ($h1) {
				$category_info[$h1] = str_replace('{filter}', '', $category_info[$h1]);
			} else {
				$category_info['name'] = str_replace('{filter}', '', $category_info['name']); // A! OpenCart Initial - for catalog ONLY
			}

			$category_info['description'] = str_replace('{filter}', '', $category_info['description']);
			
			# Вырезание двойных пробелов
			// Вообще, они уже вырезались при парсинге моей библиотекой. Но после удаления {filter}, они снова есть
			// И, возможно, вырезание пробелов понадобится для стыковки с дургим модулем позже...
			$category_info['meta_title'] = preg_replace('/\s+/', ' ', $category_info['meta_title']);
			$category_info['meta_description'] = preg_replace('/\s+/', ' ', $category_info['meta_description']);
			$category_info['meta_keyword'] = preg_replace('/\s+/', ' ', $category_info['meta_keyword']);

			if ($h1) {
				$category_info[$h1] = preg_replace('/\s+/', ' ', $category_info[$h1]);
			} else {
				$category_info['name'] = preg_replace('/\s+/', ' ', $category_info['name']); // A! OpenCart Initial - for catalog ONLY
			}

			$category_info['description'] = preg_replace('/\s+/', ' ', $category_info['description']);
		}
		// For OCFilter . End
