...
class ControllerBlogArticle extends Controller {
    ...
    // SEO URL Generator PRO . begin
		...
		$data['article_id'] = isset($this->request->get['article_id']) ? $this->request->get['article_id'] : false;
		...
		$this->load->language('model/blog/article.php'); // for heading_title no conflict
		
		if (isset($this->request->get['article_id'])) {
			if (isset($this->request->post['seo_url_generator_redirects'])) {
				$data['redirects'] = $this->request->post['seo_url_generator_redirects'];
			} else {
				$data['redirects'] = $this->model_extension_module_seo_url_generator->getRedirects('article_id', $this->request->get['article_id']);
			}
		} else {
