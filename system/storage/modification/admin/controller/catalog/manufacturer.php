...
class ControllerCatalogManufacturer extends Controller {
    ...
    // SEO Tags Generator . begin
		...
		if ($data['stg_status']) {
			$data['stg_not_use_auto'] = false;

			$data['stg_preview'] = array();
			...
			if (isset($this->request->get['manufacturer_id'])) {
				...
				foreach ($data['languages'] as $language) {
					$stg_manufacturer_info0 = array(
						'manufacturer_id'	 => $this->request->get['manufacturer_id'],
						'name'						 => isset($manufacturer_info['name']) ? $manufacturer_info['name'] : '', // On branch 3.x - ocStore get this from OpenCart PRO
						'meta_title'			 => isset($data['manufacturer_description'][$language['language_id']]['meta_title']) ? $data['manufacturer_description'][$language['language_id']]['meta_title'] : '',
						'meta_description' => isset($data['manufacturer_description'][$language['language_id']]['meta_description']) ? $data['manufacturer_description'][$language['language_id']]['meta_description'] : '',
						'meta_keyword'		 => isset($data['manufacturer_description'][$language['language_id']]['meta_keyword']) ? $data['manufacturer_description'][$language['language_id']]['meta_keyword'] : '',
					);

					if (array_key_exists('meta_h1', $data['manufacturer_description'][$language['language_id']])) {
						$stg_manufacturer_info0['meta_h1'] = $data['manufacturer_description'][$language['language_id']]['meta_h1']; // ocStore
					}

					if (array_key_exists('name_synonym', $data['manufacturer_description'][$language['language_id']])) {
						$stg_manufacturer_info0['name_synonym'] = $data['manufacturer_description'][$language['language_id']]['name_synonym']; // for manufacturer_synonym
					}

					$stg_manufacturer_info1 = $this->load->controller('extension/module/seo_tags_generator/getManufacturerTags', array(
						'manufacturer_info' => $stg_manufacturer_info0,
						'language_id' => $language['language_id'],
					));

					$data['stg_preview'][$language['language_id']]['meta_title']			 = $stg_manufacturer_info1['meta_title'];
					$data['stg_preview'][$language['language_id']]['meta_description'] = $stg_manufacturer_info1['meta_description'];
					$data['stg_preview'][$language['language_id']]['meta_keyword']		 = $stg_manufacturer_info1['meta_keyword'];
					if (isset($stg_manufacturer_info1['meta_h1'])) {
						$data['stg_preview'][$language['language_id']]['meta_h1'] = $stg_manufacturer_info1['meta_h1']; // ocStore
					}
				}
			}
		}
		...
		$this->load->language('catalog/manufacturer'); // for heading_title no conflict
