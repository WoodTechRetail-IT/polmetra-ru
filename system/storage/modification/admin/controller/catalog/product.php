...
class ControllerCatalogProduct extends Controller {
	...
	public function index() {
		...
		$this->load->model('catalog/product');
				...
				$this->model_catalog_product->createMrpSQL();
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if($this->config->get('module_prodvar_status')) {
		...
		if ($this->config->get('module_admin_quick_edit_status') && $this->config->get('module_admin_quick_edit_catalog_products_status')) {
			...
			if (isset($this->request->get['filter_price_min'])) {
				$url .= '&filter_price_min=' . urlencode(html_entity_decode($this->request->get['filter_price_min'], ENT_QUOTES, 'UTF-8'));
			}
			
			if (isset($this->request->get['filter_price_max'])) {
				$url .= '&filter_price_max=' . urlencode(html_entity_decode($this->request->get['filter_price_max'], ENT_QUOTES, 'UTF-8'));
			}
			...
			if (isset($this->request->get['filter_quantity_min'])) {
				$url .= '&filter_quantity_min=' . urlencode(html_entity_decode($this->request->get['filter_quantity_min'], ENT_QUOTES, 'UTF-8'));
			}
			
			if (isset($this->request->get['filter_quantity_max'])) {
				$url .= '&filter_quantity_max=' . urlencode(html_entity_decode($this->request->get['filter_quantity_max'], ENT_QUOTES, 'UTF-8'));
			}
			...
			if (isset($this->request->get['filter_noindex'])) {
				$url .= '&filter_noindex=' . urlencode(html_entity_decode($this->request->get['filter_noindex'], ENT_QUOTES, 'UTF-8'));
			}
...
$data['all_options'] = $this->model_extension_module_image_option->getAllOptions();

if(isset($this->request->get['product_id'])) {
  $data['product_images_options'] = $this->model_extension_module_image_option->getImagesOptionsByProductId($this->request->get['product_id']);
} else {
  $data['product_images_options'] = array();
}
    ...
    // SEO URL Generator PRO . begin
		...
		$this->load->language('extension/module/seo_tags_generator'); // SEO Tags Generator

		// SEO Tags Generator . begin
		$data['entry_product_meta_stg_no_auto'] = $this->language->get('entry_product_meta_stg_no_auto');
		$data['entry_product_meta_stg_no_auto_help'] = $this->language->get('entry_product_meta_stg_no_auto_help');
		$data['text_product_explain_stg_no_auto'] = $this->language->get('text_product_explain_stg_no_auto');
		...
		if (!$data['stg_status'] || !isset($this->request->get['product_id'])) {
		...
		if ($this->model_extension_module_seo_tags_generator->notUseAutoProduct($this->request->get['product_id'])) {
			$data['stg_not_use_auto'] = true;

			goto stg_tags_preview_end;
		} // else continue ->

		$stg_product_data = array();
		
		$stg_product_data['minimum']			 = 0;
		$stg_product_data['tax_class_id']	 = 0;
		$stg_product_data['quantity']			 = 0;
		$stg_product_data['viewed']				 = 0;
		$stg_product_data['model']				 = '';
		$stg_product_data['sku']					 = '';
		$stg_product_data['upc']					 = '';
		$stg_product_data['ean']					 = '';
		$stg_product_data['jan']					 = '';
		$stg_product_data['isbn']					 = '';
		$stg_product_data['mpn']					 = '';

		if (isset($product_info)) {
			$stg_product_data['minimum']			 = $product_info['minimum'];
			$stg_product_data['tax_class_id']	 = $product_info['tax_class_id'];
			$stg_product_data['quantity']			 = $product_info['quantity'];
			$stg_product_data['viewed']				 = $product_info['viewed'];
			$stg_product_data['model']				 = $product_info['model'];
			$stg_product_data['sku']					 = $product_info['sku'];
			$stg_product_data['upc']					 = $product_info['upc'];
			$stg_product_data['ean']					 = $product_info['ean'];
			$stg_product_data['jan']					 = $product_info['jan'];
			$stg_product_data['isbn']					 = $product_info['isbn'];
			$stg_product_data['mpn']					 = $product_info['mpn'];
		} elseif ($this->request->server['REQUEST_METHOD'] == 'POST') {
			$stg_product_data['minimum']			 = $this->request->post['minimum'];
			$stg_product_data['tax_class_id']	 = $this->request->post['tax_class_id'];
			$stg_product_data['quantity']			 = $this->request->post['quantity'];
			$stg_product_data['viewed']				 = 'VIEWED'; //it is not available in POST...
			$stg_product_data['model']				 = $this->request->post['model'];
			$stg_product_data['sku']					 = $this->request->post['sku'];
			$stg_product_data['upc']					 = $this->request->post['upc'];
			$stg_product_data['ean']					 = $this->request->post['ean'];
			$stg_product_data['jan']					 = $this->request->post['jan'];
			$stg_product_data['isbn']					 = $this->request->post['isbn'];
			$stg_product_data['mpn']					 = $this->request->post['mpn'];
		}

		foreach ($data['languages'] as $language) {
			$stg_product_info0 = array(
				'product_id'			 => $this->request->get['product_id'],
				'language_id'			 => isset($language['language_id']) ? $language['language_id'] : false,
				'name'						 => isset($data['product_description'][$language['language_id']]['name']) ? $data['product_description'][$language['language_id']]['name'] : '',
				'meta_title'			 => isset($data['product_description'][$language['language_id']]['meta_title']) ? $data['product_description'][$language['language_id']]['meta_title'] : '',
				'meta_description' => isset($data['product_description'][$language['language_id']]['meta_description']) ? $data['product_description'][$language['language_id']]['meta_description'] : '',
				'meta_keyword'		 => isset($data['product_description'][$language['language_id']]['meta_keyword']) ? $data['product_description'][$language['language_id']]['meta_keyword'] : '',
				'description'			 => isset($data['product_description'][$language['language_id']]['description']) ? $data['product_description'][$language['language_id']]['description'] : '',
				//'manufacturer'		 => $data['manufacturer'], // A-M!
				'manufacturer_id'	 => $data['manufacturer_id'], // A-M!
				'model_synonym'		 => isset($data['model_synonym']) ? $data['model_synonym'] : false,
				'price'						 => $data['price'],
				'special'					 => $this->model_extension_module_seo_tags_generator->getSpecial($this->request->get['product_id']),
				'rating'					 => $this->model_extension_module_seo_tags_generator->getRating($this->request->get['product_id']),
				'reviews'					 => $this->model_extension_module_seo_tags_generator->getReviews($this->request->get['product_id']),
			);

			if (array_key_exists('meta_h1', $data['product_description'][$language['language_id']])) {
				$stg_product_info0['meta_h1'] = $data['product_description'][$language['language_id']]['meta_h1']; // ocStore
			}

			if (array_key_exists('h1', $data['product_description'][$language['language_id']])) {
				$stg_product_info0['h1'] = $data['product_description'][$language['language_id']]['h1']; // seo-tag-h1.ocmod.zip
			}

			$stg_product_info0 = array_merge($stg_product_data, $stg_product_info0);

			$stg_product_info1 = $this->load->controller('extension/module/seo_tags_generator/getProductTags', array(
				'product_info' => $stg_product_info0,
				'language_id' => $language['language_id'],
				'attribute_groups' => $this->model_extension_module_seo_tags_generator->getProductAttributes($this->request->get['product_id']),
			));

			$data['stg_preview'][$language['language_id']]['meta_title']			 = $stg_product_info1['meta_title'];
			$data['stg_preview'][$language['language_id']]['meta_description'] = $stg_product_info1['meta_description'];
			$data['stg_preview'][$language['language_id']]['meta_keyword']		 = $stg_product_info1['meta_keyword'];

			if (isset($stg_product_info1['meta_h1'])) {
				$data['stg_preview'][$language['language_id']]['meta_h1'] = $stg_product_info1['meta_h1'];	// ocStore
			}

			if (isset($stg_product_info1['h1'])) {
				$data['stg_preview'][$language['language_id']]['h1'] = $stg_product_info1['h1']; // seo-tag-h1.ocmod.zip
			}
		}
        ...
        // << Quantity per Option
				'default_quantity' => isset($product_option_value['default_quantity']) ? (int)$product_option_value['default_quantity'] : '',
        // >> Quantity per Option
		...
		$this->load->language('catalog/product'); // for heading_title no conflict
		...
		if (isset($this->request->post['product_seo_url'])) {
			...
			$data['product_seo_url'] = array();
				...
				if (isset($this->request->post['product_mrp'])) {
					$product_mrps = $this->request->post['product_mrp'];
				} elseif (isset($this->request->get['product_id'])) {
					$product_mrps = $this->model_catalog_product->getproductTables($this->request->get['product_id']);
				} else {
					$product_mrps = array();
				}
				...
				foreach ($product_mrps as $product_mrp) {
					$data['product_mrps'][] = array(
						'product_mrp_id'          		=> (isset($product_mrp['product_mrp_id'])) ? $product_mrp['product_mrp_id'] : 0,
						'product_mrp_product'     		=> (isset($product_mrp['product_mrp_product'])) ? $product_mrp['product_mrp_product'] : array(),
						'product_mrp_description' 		=> $product_mrp['product_mrp_description'],
						'sort_order'                 	=> $product_mrp['sort_order'],
						'status'                     	=> $product_mrp['status'],
					);
				}
