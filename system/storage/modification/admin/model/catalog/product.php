...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . $data['quantity'] . "', minimum = '" . $data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', noindex = '" . (int)$data['noindex'] . "', tax_class_id = '" . (int)$data['tax_class_id'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW(), date_modified = NOW()");
    ...
    // SEO Tags Generator . end
				...
				if (isset($data['product_mrp'])) {
					foreach ($data['product_mrp'] as $product_mrp) {
						$this->db->query("INSERT INTO " . DB_PREFIX . "product_mrp SET product_id = '" . (int)$product_id . "', status = '" . (int)$product_mrp['status'] . "', sort_order = '" . (int)$product_mrp['sort_order'] . "'");

						$product_mrp_id = $this->db->getLastId();

						foreach ($product_mrp['product_mrp_description'] as $language_id => $product_mrp_description) {				
							$this->db->query("INSERT INTO " . DB_PREFIX . "product_mrp_description SET product_mrp_id = '" . (int)$product_mrp_id . "', language_id = '" . (int)$language_id . "', product_id = '" . (int)$product_id . "', description = '" .  $this->db->escape($product_mrp_description['description']) . "', title = '" .  $this->db->escape($product_mrp_description['title']) . "'");
						}
						
						if (isset($product_mrp['product_mrp_product'])) {
							foreach ($product_mrp['product_mrp_product'] as $product_mrp_product) {
								$this->db->query("INSERT INTO " . DB_PREFIX . "product_mrp_product SET product_mrp_id = '" . (int)$product_mrp_id . "', product_id = '" . (int)$product_id . "', sort_order = '" . (int)$product_mrp_product['sort_order'] . "', related_id = '" . (int)$product_mrp_product['related_id'] . "'");
							}
						}
					}
				}
...
if (isset($data['product_image'])) {
  foreach ($data['product_image'] as $key => $product_image) {  
				...
				if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
					if (isset($product_option['product_option_value'])) {
						...
						foreach ($product_option['product_option_value'] as $product_option_value) {
							$this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value SET product_option_id = '" . (int)$product_option_id . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', option_value_id = '" . (int)$product_option_value['option_value_id'] . "', quantity = '" . $product_option_value['quantity'] . "', subtract = '" . (int)$product_option_value['subtract'] . "', price = '" . (float)$product_option_value['price'] . "', price_prefix = '" . $this->db->escape($product_option_value['price_prefix']) . "', points = '" . (int)$product_option_value['points'] . "', points_prefix = '" . $this->db->escape($product_option_value['points_prefix']) . "', weight = '" . (float)$product_option_value['weight'] . "', weight_prefix = '" . $this->db->escape($product_option_value['weight_prefix']) . "'");

        // << Quantity per Option
				$product_option_value_id = $this->db->getLastId();
        \liveopencart\ext\qpo::getInstance($this->registry);
				$this->liveopencart_ext_qpo->setPOVDefaultQuantity($product_option_value_id, $product_option_value);
        // >> Quantity per Option
    ...
    // SEO Tags Generator . end
				...
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_mrp WHERE product_id = '" . (int)$product_id . "'");
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_mrp_description WHERE product_id = '" . (int)$product_id . "'");
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_mrp_product WHERE product_id = '" . (int)$product_id . "'");
			...
			foreach ($data['product_option'] as $product_option) {
				if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
					if (isset($product_option['product_option_value'])) {
						...
						foreach ($product_option['product_option_value'] as $product_option_value) {
							$this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value SET product_option_value_id = '" . (int)$product_option_value['product_option_value_id'] . "', product_option_id = '" . (int)$product_option_id . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', option_value_id = '" . (int)$product_option_value['option_value_id'] . "', quantity = '" . $product_option_value['quantity'] . "', subtract = '" . (int)$product_option_value['subtract'] . "', price = '" . (float)$product_option_value['price'] . "', price_prefix = '" . $this->db->escape($product_option_value['price_prefix']) . "', points = '" . (int)$product_option_value['points'] . "', points_prefix = '" . $this->db->escape($product_option_value['points_prefix']) . "', weight = '" . (float)$product_option_value['weight'] . "', weight_prefix = '" . $this->db->escape($product_option_value['weight_prefix']) . "'");
...
$this->db->query("DELETE FROM " . DB_PREFIX . "product_image_option WHERE product_id = '" . (int)$product_id . "'");
	...
	public function nogetProduct($product_id) {
	...
	public function getProductDownloads($product_id) {
		...
		return $product_download_data;
				...
				public function getproductTables($product_id) {
					...
					$product_mrp_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_mrp WHERE product_id = '" . (int)$product_id . "' ORDER BY sort_order ASC");

					foreach ($product_mrp_query->rows as $product_mrp) {
						$product_mrp_description_data = array();

						$product_mrp_description_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_mrp_description WHERE product_mrp_id = '" . (int)$product_mrp['product_mrp_id'] . "'");

						foreach ($product_mrp_description_query->rows as $product_mrp_description) {			
							$product_mrp_description_data[$product_mrp_description['language_id']] = array(
								'title' 		=> $product_mrp_description['title'],
								'description' 	=> $product_mrp_description['description'],
							);
						}

						$product_mrp_product_data = array();

						$product_mrp_product_data_q = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_mrp_product WHERE product_mrp_id = '" . (int)$product_mrp['product_mrp_id'] . "' AND product_id = '" . (int)$product_id . "' ORDER BY sort_order, LCASE(product_mrp_related_id)");

						foreach ($product_mrp_product_data_q->rows as $product_mrp_product) {
							$product_info = $this->model_catalog_product->getProduct($product_mrp_product['related_id']);

							$product_mrp_product_data[] = array(
								'product_mrp_related_id' => $product_mrp_product['product_mrp_related_id'],
								'sort_order'                    => $product_mrp_product['sort_order'],
								'related_id'                => $product_mrp_product['related_id'],
								'product_name'              => (!empty($product_info)) ? $product_info['name'] : ''
							);
						}

						$product_mrp_data[] = array(
							'product_mrp_description' => $product_mrp_description_data,
							'status'                     => $product_mrp['status'],
							'sort_order'                 => $product_mrp['sort_order'],
							'product_mrp_id'          => $product_mrp['product_mrp_id'],
							'product_mrp_product'     => $product_mrp_product_data,						
						);
					}
				...
				public function createMrpSQL() {
					$query = $this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "product_mrp` (
					`product_mrp_id` int(11) NOT NULL AUTO_INCREMENT, 
					`product_id` int(11) NOT NULL,
					`sort_order` int(3) NOT NULL DEFAULT '0',
					`status` int(1) NOT NULL,
					PRIMARY KEY (`product_mrp_id`)
					) ENGINE=MyISAM DEFAULT CHARSET=utf8 AUTO_INCREMENT=1 ; 
					");

					$query = $this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "product_mrp_description` (
						`product_mrp_id` int(11) NOT NULL,
						`language_id` int(11) NOT NULL,
						`product_id` int(11) NOT NULL,
						`title` varchar(255) CHARACTER SET utf8 NOT NULL,
						`description` text NOT NULL,
						PRIMARY KEY (`product_mrp_id`, `language_id`) 
					) ENGINE=MyISAM DEFAULT CHARSET=utf8; 
					");

					$query = $this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "product_mrp_product` (
						`product_mrp_related_id` int(11) NOT NULL AUTO_INCREMENT,
						`product_id` int(11) NOT NULL,
						`product_mrp_id` int(11) NOT NULL,
						`sort_order` int(11) NOT NULL,
						`related_id` int(11) NOT NULL,
						PRIMARY KEY (`product_mrp_related_id`) 
					) ENGINE=MyISAM DEFAULT CHARSET=utf8; 
					");
				}
